...
def GAN_training_function(G, D, GD, z_, y_, ema, state_dict, config):
  def train(x, y):
    ...
    if config['G_ortho'] > 0.0:
      ...
      utils.ortho(G, config['G_ortho'], blacklist=list(G.shared.parameters()))
    G.optim.step()
...
def save_and_sample(G, D, G_ema, z_, y_, fixed_z, fixed_y, 
  ...
  if not os.path.isdir(f"{config['samples_root']}/{experiment_name}"):
    os.mkdir(f"{config['samples_root']}/{experiment_name}")
...
def test(G, D, G_ema, z_, y_, state_dict, config, sample, get_inception_metrics,
  ...
  if ((config['which_best'] == 'IS' and IS_mean > state_dict['best_IS'])
    ...
    print(
        f"{config['which_best']} improved over previous best, saving checkpoint..."
    )
    utils.save_weights(G, D, state_dict, config['weights_root'],
