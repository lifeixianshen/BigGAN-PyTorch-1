...
def run(config):
  ...
  if config['G_eval_mode']:
    ...
    print(f"G is in {'training' if G.training else 'eval'} mode...")
  ...
  if config['sample_npz']:
    ...
    npz_filename = f"{config['samples_root']}/{experiment_name}/samples.npz"
    print(f'Saving npz to {npz_filename}...')
  ...
  if config['sample_random']:
    ...
    torchvision.utils.save_image(
        images.float(),
        f"{config['samples_root']}/{experiment_name}/random_samples.jpg",
  ...
  def get_metrics():
    ...
    outstring = f"Using {'ema' if config['use_ema'] else 'non-ema'} weights "
    outstring += f"in {'eval' if config['G_eval_mode'] else 'training'} mode, "
